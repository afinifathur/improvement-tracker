...
<div class="flex items-start justify-between gap-4 px-4 py-3 border-b border-outline-variant/10 last:border-b-0">
    <div class="min-w-0">
        ...
        <p class="text-[10px] text-amber-700 mt-0.5">Blocked: {{ str_replace('_', ' ', $blockedReason) }}</p>
        @endif
    </div>
    <div class="shrink-0 text-right space-y-1">
        <p class="text-xs text-on-surface-variant">{{ $item->planned_start_date->format('d M') }} – {{ $item->planned_end_date->format('d M') }}</p>
        <span class="inline-block px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider border {{ $badge }}">{{ str_replace('_', ' ', $statusValue) }}</span>
    </div>
</div>
