...
@section('title', 'Kaizen Tracker | This Week View')
...
<div class="p-6 space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between border-b border-slate-200 pb-4 gap-4">
        ...
        <form method="GET" action="{{ route('work-items.this-week') }}" id="date-nav-form" class="flex items-center gap-1.5">
            ...
            <a href="{{ route('work-items.this-week', array_merge(request()->query(), ['date' => $prevWeekDate])) }}" class="flex items-center justify-center px-2 h-7 bg-white border border-slate-200 rounded text-[11px] font-bold text-slate-600 hover:bg-slate-50 transition-colors uppercase tracking-wider gap-1">
                <span class="material-symbols-outlined text-[16px]">chevron_left</span> Prev Week
            </a>
            ...
            <a href="{{ route('work-items.this-week', array_merge(request()->query(), ['date' => $nextWeekDate])) }}" class="flex items-center justify-center px-2 h-7 bg-white border border-slate-200 rounded text-[11px] font-bold text-slate-600 hover:bg-slate-50 transition-colors uppercase tracking-wider gap-1">
                Next Week <span class="material-symbols-outlined text-[16px]">chevron_right</span>
            </a>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-5 gap-4">
        <div class="bg-white p-4 border border-slate-200 rounded-sm shadow-sm flex flex-col justify-between">
            <span class="text-[9px] font-bold uppercase tracking-wider text-slate-400">PLANNED</span>
    ...
    <div class="space-y-4">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @elseif($item->classification === 'upcoming')
                                                <span class="px-1.5 py-0.5 bg-slate-50 text-slate-500 border border-slate-200/50 rounded-[2px] text-[9px] font-extrabold uppercase tracking-wide">UPCOMING</span>
                                            @elseif($item->classification === 'completed')
                                                <span class="px-1.5 py-0.5 bg-green-50 text-green-700 border border-green-200/50 rounded-[2px] text-[9px] font-extrabold uppercase tracking-wide">COMPLETED</span>
                                            @elseif($item->classification === 'blocked')
                                                <span class="px-1.5 py-0.5 bg-amber-50 text-amber-700 border border-amber-200/50 rounded-[2px] text-[9px] font-extrabold uppercase tracking-wide">BLOCKED</span>
                                            @else
                                                <span class="px-1.5 py-0.5 bg-slate-100 text-slate-400 border border-slate-200/50 rounded-[2px] text-[9px] font-extrabold uppercase tracking-wide">CANCELLED</span>
                                            @endif
                                        </td>
                                        <td>
                                            <div class="flex items-center gap-2">
        ...
        @if(!$hasGroupedItems)
            <div class="text-center py-12 bg-white border border-slate-200 rounded-sm">
                ...
                <p class="text-xs text-slate-400 font-bold uppercase tracking-wider">No active work items matching filters for this week</p>
            </div>
        @endif
    </div>
</div>
