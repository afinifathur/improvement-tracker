...
<div class="p-6 space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between border-b border-slate-200 pb-4 gap-4">
        ...
        <form method="GET" action="{{ route('work-items.completed') }}" class="flex items-center gap-1.5">
    ...
    <!-- Filter Bar -->
    <form method="GET" action="{{ route('work-items.completed') }}" class="filter-bar flex-wrap gap-3">
        @if($date)
                            ...
                            <tbody>
                                @foreach($areaItems as $item)
                                    <tr class="opacity-80">
                                        ...
                                        <td>
                                            <span class="text-slate-400 text-[10px]">{{ $item->planned_start_date ? $item->planned_start_date->format('d M Y') : '-' }}</span>
                                        </td>
                                        <td>
                                            <span class="text-slate-400 text-[10px]">{{ $item->planned_end_date ? $item->planned_end_date->format('d M Y') : '-' }}</span>
                                        </td>
                                        <td>
                                            <span class="text-green-700 font-bold text-[10px]">
                                                {{ $item->completed_at ? $item->completed_at->format('d M Y') : ($item->planned_end_date ? $item->planned_end_date->format('d M Y') : '-') }}
                                            </span>
                                        </td>
                                        <td>
